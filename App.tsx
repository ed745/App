...
const App: React.FC = () => {
  // --- PERSISTENCIA DE DATOS (ARCHIVO INTERNO) ---
  ...
  useEffect(() => {
    localStorage.setItem('ff_currency', selectedCurrency);
  }, [selectedCurrency]);
  ...
  }, [categories, getChildren]); // Se añade getChildren a dependencias para mayor precisión
  ...
  const deleteCategory = (cat: FinanceCategory) => {
    ...
    // Mensaje de confirmación exacto solicitado
                ...
                <div className="flex flex-col">
                    ...
                    {cat.type === 'expense' && cat.sourceIncomeId && (
                        <span className="text-[9px] font-black text-[#13ec6a] uppercase flex items-center gap-1 mt-0.5">
                            ...
                            Origen: {categories.find(c => c.id === cat.sourceIncomeId)?.name || 'Desconocido'}
                        </span>
                    )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-2 pl-2 border-l border-gray-100">
                <button 
                  ...
                  className="text-gray-400 hover:text-[#13ec6a] transition-colors active:scale-90"
      ...
      <div className="max-w-md mx-auto bg-[#f6f8f7] shadow-xl min-h-screen flex flex-col">
        ...
        <header className="px-6 pt-10 pb-4 flex items-center justify-between sticky top-0 bg-[#f6f8f7]/90 backdrop-blur-md z-20">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={() => {
                if(confirm('¿Reiniciar todos los datos al estado inicial?')) {
                  localStorage.clear();
                  window.location.reload();
                }
              }}
              className="text-gray-400 hover:text-red-500"
              title="Borrar todo el almacenamiento interno"
            >
            ...
            {isAdding && (
              <div className="bg-white border-2 border-[#13ec6a] rounded-2xl p-4 shadow-xl animate-in zoom-in duration-200 flex flex-col gap-3">
                ...
                {isAdding.type === 'expense' && (
                    <div className="flex flex-col gap-1">
                        <label className="text-[9px] font-black text-gray-400 uppercase">Origen de los Fondos (Garantía de Balance)</label>
                        <select 
                ...
                <div className="flex gap-2">
                   <button onClick={handleAddCategory} className="flex-1 bg-[#0d1b13] text-white py-2 rounded-xl text-xs font-bold shadow-md active:scale-95">Crear</button>
